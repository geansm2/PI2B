...
Síndrome Gripal (SG) oriundas a partir da incorporação do sistema e-SUS
VE Notifica, desenvolvido para o registro de casos suspeitos de
COVID-19. O conjunto de dados selecionados restringe ao volume 1 dos
dados coletados no Estado do Paraná.

### Objetivos

O objetivo é fazer uma análise exploratória dos dados, analisando grupos
catalogados, avaliar a amplitudes dos dados coletados e visualizar o
comportamento dos dados através das ferramentas gráficas significativas
com as variáveis disponíveis.

### Analise exploratoria de dados.

Para facilitar o processo de importarção dos dados da planilha no
formato .CSV, escolhe-se a fonte de dados direto do direterório.
...
Filtra-se as variáveis de interesse.
...
Os nomes de cada coluna indica o tipo de observação catalogada são elas:
estadoTeste, dataTeste, tipoTeste, resultadoTeste, sexo,
...
*null*, para melhor manipular as observações, identifica-se esses
valores por atribuição de `NA` e ao fim remove-se as linhas que contém
essa característica.
...
Perceba que existe ocorrência de teste realizado no ano de
1970/2000/2002 na coluna *dataTeste*, e 2002/2019/2011/2010 na coluna
*dataEncerramento*. Na coluna da dato de teste possivelmente foi
...
O mesmo para o registro do encerramento do acomapanhamento
...
Feitos esses ajuste, é possivel identificar que o registro mais antigo
nos registro de acompanhamento é de 2020-02-10T03:00:00.000Z sendo o
ultimo em 2021-01-23T03:00:00.000Z. Fica facil de visualizar a
frequência acumulada pelo gráfico aseguir:
...
da função `boxplot()` as características como mediana, amplitude de
idades máximas e mínimas e ainda representada per quartis.
...
Ainda sobre a variável *idade*, é possivel observar que os valores
amostrados tem uma distribução similar a distribuição normal
...
observado, esta análise descritiva é importante no que diz respeito aos
estudos cientifico e análise estatística com fins de tomada de decisões.
