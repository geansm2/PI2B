...
Feitos esses ajuste, é possivel identificar que o registro mais antigo nos registro de acompanhamento é de `r dados$dataTeste[order(dados$dataTeste)[1]]` sendo o ultimo em `r dados$dataTeste[order(dados$dataTeste)[nrow(dados)]]`. Fica facil de visualizar a frequência acumulada pelo gráfico aseguir:  
...
Ainda existe mais divergência nos dados amostrado no banco de dados, nos valores indicado no campo idade, tendo um intervalor de `r min(range(dados$idade))` a `r max(range(dados$idade))`, o que não é possível no mundo real.
